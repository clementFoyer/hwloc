...
/** \brief Get the index of the CPU kind that contains a set of CPUs.
 *
 * Return the index of the CPU kind that contains \p cpuset.
 *
 * \return -1 with \errno set to \p EXDEV if \p cpuset is
 * only partially included in the some kind.
 * \return -1 with \errno set to \p ENOENT if \p cpuset is
